...
class LamosAdmincp {
	...
	function check_cpaccess() {
		...
		if(!$this->adminuser['uid']) {
			...
			if(!$this->isfounder) {
				$session = DB::fetch_first(
					"SELECT m.cpgroupid,  m.customperm, s.*
				...
				$session = DB::fetch_first(
					"SELECT * FROM ".DB::table('common_admincp_session')."
		...
		if($this->cpaccess == 1) {
			...
			DB::query(
				"INSERT INTO ".DB::table('common_admincp_session')." (uid, adminid, panel, ip, dateline, errorcount)
	...
	function do_request($action) {
		...
		if($action == 'main' || $this->allow($action, $operation, $do)) {
			require $this->admincpfile($action);
		} else {
			cpheader();
			cpmsg('action_noaccess', '', 'error');
		}
